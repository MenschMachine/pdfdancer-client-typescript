...
describe('Path E2E Tests (New API)', () => {
    ...
    test('change path stroke color', async () => {
        ...
        await assertions.assertPathUsesPaintOperator('PATH_0_000001', 'S');
    });
    ...
    test('change path fill color', async () => {
        ...
        await assertions.assertPathUsesPaintOperator('PATH_0_000004', 'f');
    });
    ...
    test('add stroke to fill-only path', async () => {
        ...
        await assertions.assertPathUsesPaintOperator('PATH_0_000004', 'B');
    });

    test('set stroke and fill colors simultaneously', async () => {
        const [baseUrl, token, pdfData] = await requireEnvAndFixture('basic-paths.pdf');
        const pdf = await PDFDancer.open(pdfData, token, baseUrl);
